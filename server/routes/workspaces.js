...
router.delete("/:id", getWorkspace, async (req, res) => {
     ...
     */
    try {
        const deleteWorkspace = await Workspace.findByIdAndDelete(req.workspace.id).populate({
            path: "lists",
            populate: { path: "tasks", populate: { path: "author assignees" } },
        });
...
async function getWorkspace(req, res, next) {
    ...
    try {
        workspace =
            (await Workspace.findById(req.params.id).populate({
                path: "lists",
                populate: { path: "tasks", populate: { path: "author assignees" } },
            })) ||
            (await Workspace.findById(req.params.id).populate("lists")) ||
